...
func main() {
	r := Jee.NewEngine()
	r.GET("/index", func(c *Jee.Context) {
		c.HTML(http.StatusOK, "<h1>Index Page</h1>")
	})
	v1 := r.Group("/v1")
	{
		v1.GET("/", func(c *Jee.Context) {
			c.HTML(http.StatusOK, "<h1>Hello Gee</h1>")
		})

		v1.GET("/hello", func(c *Jee.Context) {
	...
	v2 := r.Group("/v2")
	{
		v2.GET("/hello/:name", func(c *Jee.Context) {
			// expect /hello/geektutu
			c.String(http.StatusOK, "hello %s, you're at %s\n", c.Param("name"), c.Path)
		})
		v2.POST("/login", func(c *Jee.Context) {
